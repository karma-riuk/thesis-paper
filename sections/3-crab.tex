...
The initial pipeline run on the 398 pre-filtered repositories produced a total of 1,681 pull
requests that satisfied the baseline conditions for extraction. These PRs met the structural
requirements outlined in the data collection process: a single code review comment and a reply from
the PR author.

Table~\ref{tab:initial-distribution} summarizes the distribution of these PRs by technical
eligibility. Notably, only a small portion of PRs both compiled and passed tests, and an even
smaller subset met the coverage criterion.
...
\begin{table}[ht]
	...
	\begin{tabular}{lrr}
		...
		\midrule
		Total dataset                             & 1681           & 100.00\%            \\
		Not code-related                          & 520            & 30.93\%             \\
		Builds and passes tests, not covered      & 151            & 8.98\%              \\
		Builds and passes tests, covered by tests & 0              & 0.00\%              \\
		\bottomrule
	\end{tabular}
	\caption{Breakdown of initial dataset (1,681 PRs)}
	\label{tab:initial-distribution}
\end{table}

Out of the 1,681 PRs, 985 were manually selected as suitable for the comment generation task. This
corresponds to 58.59\% of the total. Table~\ref{tab:manual-selection-distribution} shows how this
filtered subset compares internally in terms of technical readiness.
...
\begin{table}[ht]
	...
	\begin{tabular}{lrr}
		...
		\textbf{Subset}         & \textbf{Count} & \textbf{Percentage} \\
		\midrule
		Selected subset         & 985            & 100.00\%            \\
		Not code-related        & 322            & 32.69\%             \\
		Builds and passes tests & 101            & 10.25\%             \\
		Covered by tests        & 0              & 0.00\%              \\
		\bottomrule
	\end{tabular}
	\caption{Breakdown of manually selected subset (985 PRs)}
	\label{tab:manual-selection-distribution}
\end{table}

While the first run served its purpose in validating the pipeline and understanding failure modes,
it also revealed a major limitation: only a very small number of PRs met the coverage requirement.
This fell short of the initial objective of supporting tasks like code refinement, which rely on
coverage data. As a result, it became clear that running the pipeline on a larger pool of
repositories was necessary to obtain a meaningful number of eligible PRs.
...
\begin{table}[ht]
	...
	\begin{tabular}{lrr}
		...
		\midrule
		Failed to compile                         & 341            & 20.29\%             \\
		Commented file not in any coverage report & 167            & 9.93\%              \\
		Failed to test                            & 131            & 7.79\%              \\
		Couldn't fetch PR's merge commit          & 117            & 6.96\%              \\
		Couldn't get diffs after last commit      & 84             & 5.00\%              \\
		Couldn't execute coverage tool (JaCoCo)   & 62             & 3.69\%              \\
		No tests found                            & 56             & 3.33\%              \\
		Failed to extract test results            & 17             & 1.01\%              \\
		Other issues                              & 35             & 2.08\%              \\
		\bottomrule
	\end{tabular}
	\caption{Most common failure reasons in initial dataset (1,681 PRs)}
...
% - Valid PRs by new criteria
% - No manual selection, but inference from previous run
% - Notable differences (e.g., increase in coverage-qualified PRs)

\subsubsection{Interpretation}
% Compare proportions where appropriate
% Comment on how small dataset shaped expectations
% Mention further potential (e.g., writing tests to increase eligibility)
